...
def body2mark(real_body):
    ...
    if body.get("blocks"):
        ...
        for block in body.get("blocks"):
            b_type = block['type']
            if b_type == "p":
                styls = block.get("styles") # TODO: Figure out styles once I have a sample of some reallt weird styling.
                if styls:
                    pass
...
async def main():
    ...
    async with aiohttp.ClientSession(cookies={"FANBOXSESSID":cookie}) as session:
                        ...
                        if j_data.get("body"):
                            for post in j_data.get("body")["items"]:
                                if post['isRestricted']:
                                    #print(post)
                                    print(f"Post {post['id']} from {page_c} is not unlocked. Skipping...")
                                    continue
                                if post['id'] in running_set:
                                    print(f"Post {post['id']} from {page_c} already has been indexed.")
                                    continue
        ...
        for creator in users:
            pages = await session.get(f"https://api.fanbox.cc/post.paginateCreator?creatorId={creator}")
            if pages.status == 200:
